"""
MIL_structures Testing   Script  ver： Nov 26th 14:00
"""
    ...
    for patch_image, patch_labels, labels in test_dataloader:  # use different dataloder in different phase
        ...
        if index % check_minibatch == check_minibatch - 1:
            ...
            if enable_attention_check:
                try:
                    if not MIL_Stripe:
                        check_SAA(CLS_bag_image, labels, model, model_idx, edge_size, class_names, model_type='MIL_structures',
...
def main(args):
    if args.paint:
        # by default: use Agg kernal, not painting in the front-desk
        import matplotlib
        matplotlib.use('Agg')
    ...
    # CLS step MIL_structures
    CLS_MIL = False if args.CLS_MIL_off else True  # True

    # shuffling method
    same_cls_shuffle = args.same_cls_shuffle  # default False

    MIL_Stripe = args.MIL_Stripe  # remove MIL_structures head and analysis the CLS branch
    # fixme why when MIL_structures head is used, CAM is wrong? Youdan Feng: ..
    if not MIL_Stripe:
...
def get_args_parser():
    ...
    # no-shuffle step (CLS step) MIL_structures head, set --CLS_MIL_off to use only CLS head in the non-shuffle step
    ...
    parser.add_argument('--paint', action='store_false', help='paint in front desk')  # default matplotlib.use('Agg')
