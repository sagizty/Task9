"""
Training     Script  ver： Jun 26th 20:00
...
def main(args):
    ...
    draw_path = os.path.join(draw_root, 'CLS_' + model_idx)  # CLS_ is for the CLS training, MIL_structures will be MIL_structures training
    ...
    if confusing_training:
        datasets = {'train': AmbiguousImageFolderDataset(os.path.join(dataroot, 'train'), data_transforms['train']),
                    'val': torchvision.datasets.ImageFolder(os.path.join(dataroot, 'val'), data_transforms['val'])}
    else:
        datasets = {x: torchvision.datasets.ImageFolder(os.path.join(dataroot, x), data_transforms[x]) for x in
                    ['train', 'val']}  # 2 dataset obj is prepared here and combined
    ...
    class_names = [d.name for d in os.scandir(os.path.join(dataroot, 'val')) if d.is_dir()]
    class_names.sort()
    if num_classes == 0:
